...
public class GermanLexicon extends LexiconImpl {
    ...
    private GermanLexicon (URL compiledURL, URL addendaURL, URL letterToSoundURL, boolean binary) {
    ...
    private GermanLexicon () {
		this("germanlex");
	}

	/** Creates the CMU Lexicon which is a binary lexicon
	 * @param basename the basename for the lexicon. */
    private GermanLexicon (String basename) {
		this(basename, true);
	}

	private GermanLexicon (String basename, boolean useBinaryIO) {
	 ...
	 * @param useBinaryIO if true use binary IO to load DB
	 * @throws IOException if problems occurred while reading the data */
	private static GermanLexicon getInstance (String basename, boolean useBinaryIO) throws IOException {
