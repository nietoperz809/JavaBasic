...
class FreeTTSTime extends FreeTTS {
    ...
    private FreeTTSTime (Voice voice) {
		super(voice);
	}
	...
	private static String timeToString (String time) {
		String theTime = null;
	...
	private static String timeToString (int hour, int min) {
    ...
    private void timeToSpeech (String time) {
    ...
    private void timeToSpeech (int hour, int min) {
		if (hour < 0 || hour > 23) {
	 ...
	 * @param time the time in the form HH:MM */
    private void safeTimeToSpeech (String time) {
		try {
			if (time.equals("now")) {
				speakNow();
			} else {
