...
class USEnglish {
	...
	private static final String RX_DEFAULT_US_EN_WHITESPACE = "[ \n\t\r]+";
	/** default letter regular expression pattern */
	private static final String RX_DEFAULT_US_EN_ALPHABET = "[A-Za-z]+";
	/** default uppercase regular expression pattern */
	private static final String RX_DEFAULT_US_EN_UPPERCASE = "[A-Z]+";
	/** default lowercase regular expression pattern */
	private static final String RX_DEFAULT_US_EN_LOWERCASE = "[a-z]+";
	/** default alpha-numeric regular expression pattern */
	private static final String RX_DEFAULT_US_EN_ALPHANUMERIC = "[0-9A-Za-z]+";
	/** default identifier regular expression pattern */
	private static final String RX_DEFAULT_US_EN_IDENTIFIER = "[A-Za-z_][0-9A-Za-z_]+";
	/** default integer regular expression pattern */
	private static final String RX_DEFAULT_US_EN_INT = "-?[0-9]+";
	/** default double regular expression pattern */
	private static final String RX_DEFAULT_US_EN_DOUBLE = "-?(([0-9]+\\.[0-9]*)|([0-9]+)|(\\.[0-9]+))([eE][---+]?[0-9]+)?";
	/** default integer with commas regular expression pattern */
	private static final String RX_DEFAULT_US_EN_COMMAINT = "[0-9][0-9]?[0-9]?,([0-9][0-9][0-9],)*[0-9][0-9][0-9](\\.[0-9]+)?";
	/** default digits regular expression pattern */
	private static final String RX_DEFAULT_US_EN_DIGITS = "[0-9][0-9]*";
	/** default dotted abbreviation regular expression pattern */
	private static final String RX_DEFAULT_US_EN_DOTTED_ABBREV = "([A-Za-z]\\.)*[A-Za-z]";
	/** default ordinal number regular expression pattern */
	private static final String RX_DEFAULT_US_EN_ORDINAL_NUMBER = "[0-9][0-9,]*(th|TH|st|ST|nd|ND|rd|RD)";
	/** default has-vowel regular expression */
	private static final String RX_DEFAULT_HAS_VOWEL = ".*[aeiouAEIOU].*";
	/** default US money regular expression */
	private static final String RX_DEFAULT_US_MONEY = "\\$[0-9,]+(\\.[0-9]+)?";
	/** default -illion regular expression */
	private static final String RX_DEFAULT_ILLION = ".*illion";
	/** default digits2dash (e.g. 999-999-999) regular expression */
	private static final String RX_DEFAULT_DIGITS2DASH = "[0-9]+(-[0-9]+)(-[0-9]+)+";
	/** default digits/digits (e.g. 999/999) regular expression */
	private static final String RX_DEFAULT_DIGITSSLASHDIGITS = "[0-9]+/[0-9]+";
	/** default number time regular expression */
	private static final String RX_DEFAULT_NUMBER_TIME = "((0[0-2])|(1[0-9])):([0-5][0-9])";
	/** default Roman numerals regular expression */
	private static final String RX_DEFAULT_ROMAN_NUMBER = "(II?I?|IV|VI?I?I?|IX|X[VIX]*)";
	/** default drst "Dr. St" regular expression */
	private static final String RX_DEFAULT_DRST = "([dD][Rr]|[Ss][Tt])";
	/** default numess */
	private static final String RX_DEFAULT_NUMESS = "[0-9]+s";
	/** default 7-digit phone number */
	private static final String RX_DEFAULT_SEVEN_DIGIT_PHONE_NUMBER = "[0-9][0-9][0-9]-[0-9][0-9][0-9][0-9]";
	/** default 4-digit number */
	private static final String RX_DEFAULT_FOUR_DIGIT = "[0-9][0-9][0-9][0-9]";
	/** default 3-digit number */
	private static final String RX_DEFAULT_THREE_DIGIT = "[0-9][0-9][0-9]";
