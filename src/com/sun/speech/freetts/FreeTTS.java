...
class FreeTTS {
	...
	/** Version number. */
	private final static String VERSION = "FreeTTS 1.2.2";
	private Voice voice;
    ...
    boolean textToSpeech (String text) {
    ...
    private boolean streamToSpeech (InputStream is) {
		boolean ok;
		voice.startBatch();
		ok = voice.speak(is);
		voice.endBatch();
	 ...
	 * @return true if the utterance was played properly */
    private boolean urlToSpeech (String urlPath) {
		boolean ok = false;
		try {
	 ...
	 * @return true if the utterance was played properly */
    private boolean fileToSpeech (String filePath) {
		boolean ok = false;
		try {
    ...
    private void setMultiAudio (boolean multiAudio) {
    ...
    private void setStreamingAudio (boolean streamingAudio) {
		this.streamingAudio = streamingAudio;
	}

	/** Prints the usage message for FreeTTS. */
	private static void usage (String voices) {
